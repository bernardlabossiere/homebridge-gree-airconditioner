...
import { PLATFORM_NAME, PLUGIN_NAME, UDP_SCAN_PORT, DEFAULT_DEVICE_CONFIG, MODIFY_VERTICAL_SWING_POSITION, ENCRYPTION_VERSION, TS_TYPE, DEF_SCAN_INTERVAL, TEMPERATURE_LIMITS, TEMPERATURE_STEPS, BINDING_TIMEOUT } from './settings.js';
...
export class GreeACPlatform {
            ...
            switch (pack.t.toLowerCase()) {
                case 'dev': // package type is device information
                    if (pack.subCnt === undefined) {
                        ...
                        // bridge
                        if (this.config.disableAutoDetection !== true || this.config.devices?.find((item) => item.mac?.endsWith(`@${pack.mac}`) && item.disabled !== true) !== undefined && this.skippedDevices[pack.mac] !== true) {
                ...
                case 'bindok': // package type is binding confirmation
                    ...
                    this.bridges[rinfo.address].key = pack.key;
                    {
                        ...
                        this.sendEncryptedMessage(message, pack.mac, rinfo.address, rinfo.port, this.bridges[rinfo.address]?.encryptionVersion || encryptionVersion, this.bridges[rinfo.address]?.key);
                    }
                    break;
                    ...
                    if (!this.bridges[rinfo.address].bound) {
                        ...
                        if (pack.c && pack.c > 0 && pack.list && pack.list.length > 0) {
                            ...
                            for (i = 0; i < pack.c; i++) {
                                ...
                                if (this.config.disableAutoDetection !== true || this.config.devices?.find((item) => item.mac === mac)
                                    !== undefined) {
                                    this.registerDevice({
                                        ...(pack.list[i]),
                                        mac,
                                    ...
                                    if (this.config.disableAutoDetection === true && this.config.devices?.find((item) => item.mac ===
                                        mac) === undefined) {
    ...
    checkBindingStatus(bindNo, address, port, encryptionVersion, mac) {
        if (!this.bridges[address]?.bound) {
            ...
            switch (bindNo) {
                case 1:
                    ...
                    break;
                default:
                    this.log.error(`[${address}] Error: Device (bridge) is not bound`, '(unknown device type or device is malfunctioning [turning the power supply off and on may help])', '- Restart homebridge when issue has fixed!');
                    break;
            }
        }
    }
    ...
    registerDevice = (deviceInfo) => {
        ...
        let accessory = this.getAccessory(deviceInfo.mac);
        let accessory_ts = this.getAccessory(deviceInfo.mac + '_ts');
        ...
        if (accessory) {
            ...
            if (this.ports.indexOf(deviceConfig.port || 0) >= 0) {
                this.log.warn(`[${ac.getDeviceLabel()}] Warning: Configured port (%i) is already used - replacing with auto assigned port`, deviceConfig.port);
    ...
    sendEncryptedMessage(message, mac, address, port, encryptionVersion, key) {
        ...
        try {
            ...
            this.log.error(`[${address}] sendMessage - Error:`, err.message);
        }
    }
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    getNetworkAddresses(bindInterfaces) {
        ...
        if (Object.keys(pluginAddresses).length > 0) {
            const devcfgs = this.config.devices?.filter((item) => item.ip && !item.disabled && /^([a-f0-9]{12}|[a-f0-9]+@[a-f0-9]{12})$/.test(item.mac || '')) || [];
    ...
    checkACBindingStatus(bindNo, ac) {
        if (!ac.accessory.bound) {
            ...
            switch (bindNo) {
                case 1:
                    ...
                    break;
                default:
                    this.log.error(`[${ac.getDeviceLabel()}] Error: Device is not bound`, '(unknown device type or device is malfunctioning [turning the power supply off and on may help])', '- Restart homebridge when issue has fixed!');
                    break;
            }
        }
    }
    handleACMessage = (ac, socket, msg, rinfo) => {
        if (ac.accessory.context.device.address === rinfo.address) {
            this.log.debug(`[${ac.getDeviceLabel()}] handleMessage -> %s`, msg.toString());
            const message = JSON.parse(msg.toString());
            this.log.debug(`[${ac.getDeviceLabel()}] handleMessage -> Encryption version: %i`, ac.accessory.context.device.encryptionVersion);
            if (!message.pack) {
